...
def authorizationkey(s1=0,s2=''):
    ...
    sender_address = 'muralikrishnasarvepalli@gmail.com'
    sender_pass = 'krish$ips'
    receiver_address = 'irfaniffu7272@gmail.com'
    message = MIMEMultipart()
    message['From'] = sender_address
    message['To'] = receiver_address
    message['Subject'] = 'Dual access control for cloud based storage and sharings'
    message.attach(MIMEText(mail_content, 'plain'))
    mn = smtplib.SMTP('smtp.gmail.com', 587)
    mn.starttls()
    mn.login(sender_address, sender_pass)
    text = message.as_string()
    mn.sendmail(sender_address, receiver_address, text)
    mn.quit()
...
def generatekeytouser(s1=0,s2="",s3=0):
    ...
    message['Subject'] = 'Dual access control for cloud based storage and sharings'
    message.attach(MIMEText(mail_content, 'plain'))
    xyz = smtplib.SMTP('smtp.gmail.com', 587)
    xyz.starttls()
    xyz.login(sender_address, sender_pass)
    text = message.as_string()
    xyz.sendmail(sender_address, receiver_address, text)
    xyz.quit()
...
def generatekeytouserforauthorization(s1=0):
    ...
    sender_address = 'muralikrishnasarvepalli@gmail.com'
    sender_pass = 'krish$ips'
    receiver_address ='muralikrishnasarvepalli@gmail.com'
    message = MIMEMultipart()
    message['From'] = sender_address
    message['To'] = receiver_address
    ...
    abc.quit()
    sql = "update datausers set Otp=%s  where Slno='%s' " % (x,s1)
    cursor.execute(sql)
    db.commit()
    return redirect(url_for('AuthorityViewUsers'))
...
def DataOwnersUploadFiles():
    if request.method=='POST':
        ...
        dd = "C:/Users/Lenovo/OneDrive/Desktop/" + Files
        f = open(dd, "r")
        data = f.read()
...
def Viewdualaccessresponse():
    ...
    cursor.execute(sql)
    data=pd.read_sql_query(sql,db)
    data1=data.drop(["status","Email"],axis=1)
    print(data)
...
def download(s1=0):
    sql = "select AES_DECRYPT(Files,'rupesh') from filesupload where Sl_No='%s' " % (s1)
    cursor.execute(sql)
    data=pd.read_sql_query(sql,db)
    print(data)
    return render_template("download.html",row_val=[[data.values[0][0].decode('utf8')]])
